...
def create_mlp(layer_dims, activation='relu', final_activation='linear'):
        """
        ...
        for i in range(len(layer_dims)-1):
            layers.append(snt.Linear(layer_dims[i+1]))
            if i != len(layer_dims)- 2:
                layers.append(tf.keras.layers.Activation(activation))
            else:
                layers.append(tf.keras.layers.Activation(final_activation))
...
class DeterministicMLPActor(snt.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_dims, act_limit):
        ...
        self.pi = create_mlp(layer_dims, final_activation='tanh')
...
class MLPQFunction(snt.Module):
    ...
    def __call__(self, obs, act):
        ...
        return tf.squeeze(q, axis=1) # ensure q has the right shape


class MLPActorCritic(snt.Module):
...
class DDPGAgent:
    ...
    def __init__(self, obs_dim, act_dim, act_limit, hidden_dims=[256,128], 
        ...
        self.pi_optimizer = snt.optimizers.Adam(pi_lr)
        self.q_optimizer = snt.optimizers.Adam(q_lr)
    ...
    def compute_loss_q(self, data):
        ...
        loss_q = tf.math.reduce_mean((q - backup)**2, axis=0)
    ...
    def compute_loss_pi(self, data):
        ...
        return -tf.math.reduce_mean(q_pi, axis=0)

    def choose_action(self, obs, noisy=True):
        act = self.ac.act(tf.convert_to_tensor(obs))
        if noisy:
    ...
    def learn(self):
        ...
        params = self.ac.q.trainable_variables
        grads = tape.gradient(loss_q, params)
        self.q_optimizer.apply(grads, params)


        # Run one gradient descent step for pi
        with tf.GradientTape() as tape:
            loss_pi = self.compute_loss_pi(batch)
        params = self.ac.pi.trainable_variables
        grads = tape.gradient(loss_pi, params)
        self.pi_optimizer.apply(grads, params)

        for param, param_target in zip(self.ac.trainable_variables, self.ac_target.trainable_variables):
            # we use in place tensor operations with the trailing _
            param_target = tf.math.multiply(param_target, self.polyak)
            param_target = tf.add(param_target, tf.math.multiply(param, (1-self.polyak)))
